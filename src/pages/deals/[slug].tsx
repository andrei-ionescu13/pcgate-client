...
const getDeal = (slug: string, config: Record<string, any> = {}) => () => appFetch<DealI>({
  url: `/collections/${slug}`,
  ...config
})

const getDealProducts = (slug: string, config: Record<string, any> = {}) => ({ pageParam = 1 }) => appFetch<DealProductsPage>({
...
const Deal: NextPageWithLayout<DealProps> = (props) => {
  ...
  const { data: deal } = useQuery(['deal', slug], getDeal(slug), { initialData: dealProp });
  const {
    data,
    error,
    fetchNextPage,
      ...
      >
        ...
        <Box
          sx={{
            position: 'absolute',
            inset: 0,
            background: 'linear-gradient(90deg, rgba(0, 0, 0, 0) 50%, #161C24 100%), linear-gradient(270deg, rgba(0, 0, 0, 0) 50%, #161C24 100%), linear-gradient(180deg, rgba(0, 0, 0, 0) 0%, #161C24 100%)'
          }}
        />
      </Box>
