...
const getArticle = (slug: string, config: Record<string, any> = {}) => () => appFetch<ArticleI>({
  url: `/articles/${slug}`,
  withAuth: true,
  ...config
})
...
const Article: NextPage<ArticleProps> = (props) => {
  ...
  const { data: article } = useQuery(['article', slug], getArticle(slug));
...
export const getServerSideProps: GetServerSideProps = async ({ locale, query, req, res }) => {
  ...
  try {
    const article = await queryClient.fetchQuery(['article', slug], getArticle(slug, { req, res }))
