...
const listCollections = (config: Record<string, any> = {}) => () => appFetch<Collection[]>({
  url: '/collections',
  withAuth: true,
  ...config
})

const listNewReleases = (config: Record<string, any> = {}) => () => appFetch<Product[]>({
...
export const getServerSideProps: GetServerSideProps = async ({ locale, query, req, res }) => {
  ...
  try {
    await queryClient.fetchQuery(['collections'], listCollections({ req, res }))
    await queryClient.fetchQuery(['new-releases'], listNewReleases({ req, res }))
  } catch (error) {
    console.error(error)
  }
