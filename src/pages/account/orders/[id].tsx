...
const getUserOrder = (id: string, config: Record<string, any> = {}) => () => appFetch<OrderI>({
  url: `/auth/orders/${id}`,
  withAuth: true,
  ...config
})
...
const Order: NextPageWithLayout = () => {
  ...
  const onActivateKey = (keyValue: string, product: Product, productKey: ProductKey): void => {
    queryClient.setQueryData<OrderI>(['order', id], (prev) => {
      ...
      const order = prev;
      const item = order.lineItems.find((item) => item.productId === product._id);
  ...
  return (
    <>
      ...
      <Box>
        ...
        >
          {order.lineItems.map((item) => item.keys.map((key) => (
            <LibraryProduct
